...
function buildBotVotePrompt(o) {
  ...
  const copycatLine = `3) Copycat / piggyback: Read clues in **speaking order**. If a clue repeats the **same obvious dimension or metaphor** as an **earlier** clue—especially near-synonyms or "me too" brightness/light, heat/fire, flight, etc. (e.g. SHINE then BRIGHT)—treat the **later** echo as **more suspicious** than a player who pivots to a **different** valid angle. Innocents diversify; Imposters often piggyback. **Never** eliminate someone just because their clue does not "follow from" earlier words—good innocents add **new** facets that still fit the secret.`;

  const weakClueLine =
    roundsPlayed === 2
      ? `4) "Weak" clue test: Before you call a clue vague or inconsistent—if Innocent, ask whether it still fits the REAL word (parts, metaphors, alternate angles). If Imposter, ask whether it fits your best hypothesis from the clues; clever round-2 specificity is NOT automatically suspicious.`
      : `4) "Weak" clue test: Before you call a clue vague—if Innocent, check it against the REAL word (parts, metaphors, alternate angles). If Imposter, check against your best hypothesis; a clue that only "clicks" late can still be innocent.`;
...
${copycatLine}
${weakClueLine}
5) Counter-case: Give the strongest argument against your top suspect.
6) Runner-up: Name a second suspect and one line on why your pick beats them.
7) You MUST NOT vote for your own seat (${botSeat}).
...
function buildBotVotesBatchPrompt(o) {
  ...
  const copycatLine = `3) Copycat / piggyback: Read clues in **speaking order**. If a clue repeats the **same obvious dimension** as an **earlier** clue—near-synonyms or the same metaphor family (e.g. SHINE then BRIGHT)—treat the **later** echo as **more suspicious** than someone who opens a **different** valid angle. Innocents diversify; Imposters echo. **Never** eliminate the human (seat 0) merely because their clue feels less "chained" to prior clues—orthogonal clues are often innocent.`;
