...
- Among UNREVEALED words that are NOT ${teamLabel} (opponent, neutral, assassin only), your N targets must each be a STRICTLY stronger association to the clue than EVERY such word. Other unrevealed ${teamLabel} words you are not targeting this turn do not need to be "beaten" by the clue. If any unrevealed non-${teamLabel} word ties or beats any target, pick a different clue or targets.
...
/**
 * Second-pass check: every unrevealed non-team word must be a strictly weaker fit for the clue than every target.
 * (Unrevealed same-team words that are not targets are ignored — real Codenames risk is wrong-color guesses.)
 * @param {string[]} words
 * @param {string[]} assignment
 * @param {'blue'|'red'} team
 * @param {Record<number,string>} revealed
...
Intended ${teamLabel} words (must be strictly stronger fits than all dangerous cards below): ${targetWords}

Unrevealed NON-${teamLabel} words only (opponent, neutral, assassin — not other ${teamLabel} words still on the board): ${others.join(", ")}

Rule: a skilled Codenames player must judge that EVERY intended word is a STRICTLY STRONGER fit for the clue than EVERY word in the second list. If any word in the second list ties or beats any intended word for obviousness, reply {"ok":false}. Otherwise {"ok":true}.
