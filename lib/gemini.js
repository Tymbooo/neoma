...
function buildBlueOperativePrompt(words, revealed, clueWord, clueNumber, opts = {}) {
  ...
  const allowance = clueNumber <= 0 ? 0 : clueNumber + 1;
...
Your spymaster clue is: "${clueWord}" ${clueNumber}.
Official rule: the number on the clue is how many *blue* words it relates to; your team may make **up to (number + 1)** guesses this turn — that is **${allowance}** guesses maximum (${clueNumber} + 1 bonus guess), unless fewer covered words remain on the board.
