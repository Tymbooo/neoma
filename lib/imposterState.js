...
/** Stable digest of clue list so redeem ticket binds to exact game state. */
function canonicalCluesDigest(clues) {
  const s = (clues || [])
    .map((c) => `${c.seat}|${c.round}|${String(c.word).toUpperCase()}`)
    .join(";");
  return crypto.createHash("sha256").update(s).digest("hex");
}

/**
 * One-time redemption after the Imposter is eliminated (HMAC-signed, short TTL).
 * @param {{ token: string, clues: unknown[], imposterSeat: number, eliminated: number }} p
 */
function signRedeemTicket(p) {
  if (p.eliminated !== p.imposterSeat) return null;
  const payload = {
    exp: Math.floor(Date.now() / 1000) + 900,
    td: tokenDigest(p.token),
    cd: canonicalCluesDigest(p.clues),
    is: p.imposterSeat,
    el: p.eliminated,
  };
  const body = JSON.stringify(payload);
  const sig = crypto.createHmac("sha256", hmacSecret()).update(body).digest("hex");
  return Buffer.from(JSON.stringify({ p: payload, sig })).toString("base64url");
}

/**
 * @returns {{ exp: number, is: number, el: number } | null}
 */
function verifyRedeemTicket(ticket, token, clues) {
  if (!ticket || typeof ticket !== "string") return null;
  try {
    const o = JSON.parse(Buffer.from(ticket, "base64url").toString("utf8"));
    const { p, sig } = o;
    const body = JSON.stringify(p);
    const expect = crypto.createHmac("sha256", hmacSecret()).update(body).digest("hex");
    if (expect !== sig) return null;
    if (p.exp < Math.floor(Date.now() / 1000)) return null;
    if (tokenDigest(token) !== p.td) return null;
    if (canonicalCluesDigest(clues) !== p.cd) return null;
    if (p.is !== p.el) return null;
    if (!Number.isInteger(p.is) || p.is < 0 || p.is > 3) return null;
    return p;
  } catch {
    return null;
  }
