...
const RED_TRY_RED_PROB = 0.58;

/**
 * No LLM: up to `maxAttempts` guesses (1–3). Each step rolls "try red" vs "mistake";
...
/**
 * Full red team turn without LLM: synthetic clue + "average" random play (1–3 guesses, occasional mistakes).
 * @param {string[]} words
 * @param {string[]} assignment
 * @param {Record<number,string>} revealed
 */
function runRedTurnLocal(words, assignment, revealed) {
  ...
  const maxAttempts = 1 + Math.floor(Math.random() * 3);
